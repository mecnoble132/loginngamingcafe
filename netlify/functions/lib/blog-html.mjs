...
export function renderIndex(posts) {
  ...
  const moreHtml = more.length ? `<div class="blog-list-label"><i class="fa-solid fa-layer-group"></i> More stories</div><div class="blog-grid-index">${more.map(postCard).join('')}</div>` : '';
  ...
  const body = posts.length
    ? `<div class="blog-layout"><div class="blog-main">${featuredHtml}${secondaryHtml}${moreHtml}</div><aside class="blog-sidebar">${sidebarHtml}</aside></div>`
  ...
  return shell({ title: 'Blog | Loginn Gaming Cafe', description: 'Gaming guides, cafe stories, events, and updates from Loginn Gaming Cafe in Trivandrum.', canonical: `${SITE_URL}/blog`, body: `${hero}<section class="blog-list-section"><div class="container blog-container">${body}</div></section>${cta}` });
}
