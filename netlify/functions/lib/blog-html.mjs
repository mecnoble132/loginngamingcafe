...
function header() { return `<nav class="navbar" id="navbar"><div class="nav-container"><a href="/" class="nav-logo"><span class="logo-icon"><img src="/images/loginnlogo.png" alt="Loginn logo"></span><span class="logo-text">LOGINN<span class="accent-blue">.</span></span></a><button class="hamburger" id="hamburger" aria-label="Open navigation" aria-expanded="false"><span></span><span></span><span></span></button><ul class="nav-links" id="navLinks"><li><a href="/#pricing" class="nav-link">Pricing</a></li><li><a href="/pages/games.html" class="nav-link">Games</a></li><li><a href="/pages/membership.html" class="nav-link">Membership</a></li><li><a href="/blog" class="nav-link">Blog</a></li><li><a href="/#gallery" class="nav-link">Gallery</a></li></ul><a href="https://wa.me/918075707064" class="nav-book"><i class="fa-brands fa-whatsapp"></i> Book a Slot</a></div></nav>`; }
function footer() { return `<footer class="main-footer"><div class="container footer-container"><div class="footer-col footer-brand"><a href="/" class="nav-logo"><span class="logo-icon"><img src="/images/loginnlogo.png" alt="Loginn logo"></span><span class="logo-text">LOGINN<span class="accent-blue">.</span></span></a><p class="footer-desc">Premium gaming PCs. Zero Lag. Ice-cold vibes.<br>Your ultimate gaming destination in Thiruvananthapuram.</p></div><div class="footer-col footer-links"><h4>Quick Links</h4><ul><li><a href="/#pricing">Stations &amp; Pricing</a></li><li><a href="/pages/games.html">Games Library</a></li><li><a href="/pages/membership.html">Membership</a></li><li><a href="/blog">Blog</a></li></ul></div><div class="footer-col footer-contact"><h4>Contact Us</h4><ul><li><i class="fa-solid fa-location-dot"></i><span>Loginn Gaming Centre<br>Toll Junction, Ambalamukku<br>Thiruvananthapuram, Kerala 695003</span></li><li><i class="fa-solid fa-phone"></i><span><a href="tel:+918075707064">+91 80757 07064</a></span></li></ul></div></div><div class="footer-bottom"><div class="container footer-bottom-inner"><span class="footer-copy">© 2026 Loginn. All rights reserved.</span></div></div></footer>`; }
...
export function renderIndex(posts) {
  const cards = posts.length ? posts.map((post, index) => `<article class="blog-card"><a href="/blog/${encodeURIComponent(post.slug)}" class="blog-card-image"><img src="${escapeHtml(post.cover_image || '')}" alt="${escapeHtml(post.title)}"${index ? ' loading="lazy"' : ''}></a><div class="blog-card-content"><div class="blog-card-meta"><time datetime="${escapeHtml(post.published_at || '')}">${escapeHtml(dateText(post.published_at))}</time>${tags(post.tags)}</div><h2><a href="/blog/${encodeURIComponent(post.slug)}">${escapeHtml(post.title)}</a></h2><p>${escapeHtml(post.excerpt)}</p><a class="blog-read-link" href="/blog/${encodeURIComponent(post.slug)}">Read article <i class="fa-solid fa-arrow-right"></i></a></div></article>`).join('') : `<section class="blog-state"><i class="fa-solid fa-pen-nib"></i><h2>Stories are loading in</h2><p>There are no published posts yet. Check back soon for gaming guides, cafe updates, and event news.</p></section>`;
  return shell({ title: 'Blog | Loginn Gaming Cafe', description: 'Gaming guides, cafe stories, events, and updates from Loginn Gaming Cafe in Trivandrum.', canonical: `${SITE_URL}/blog`, body: `<section class="blog-hero"><div class="container"><span class="section-tag"><i class="fa-solid fa-bolt"></i> LATEST</span><h1>From <span class="accent-green">Loginn</span></h1><p>Guides, game nights, and everything happening at Trivandrum’s premium gaming cafe.</p></div></section><section class="blog-list-section"><div class="container"><div class="blog-grid">${cards}</div></div></section>` });
}

export function renderArticle(post) {
  const body = sanitizePostHtml(post.body_html);
  return shell({ title: `${post.title} | Loginn Gaming Cafe`, description: post.meta_description, canonical: `${SITE_URL}/blog/${post.slug}`, article: post, body: `<article class="article"><div class="container article-container"><a href="/blog" class="blog-back"><i class="fa-solid fa-arrow-left"></i> All posts</a><header class="article-header"><div class="article-tags">${tags(post.tags)}</div><h1>${escapeHtml(post.title)}</h1><p class="article-excerpt">${escapeHtml(post.excerpt)}</p><div class="article-byline"><time datetime="${escapeHtml(post.published_at || '')}">${escapeHtml(dateText(post.published_at))}</time><span></span><span>${escapeHtml(post.author_name)}</span></div></header>${post.cover_image ? `<img class="article-cover" src="${escapeHtml(post.cover_image)}" alt="${escapeHtml(post.title)}">` : ''}<div class="article-body">${body}</div><aside class="article-cta"><div><span>READY PLAYER ONE?</span><h2>Make your next game night legendary.</h2></div><a href="https://wa.me/918075707064?text=Hi!%20I%27d%20like%20to%20book%20a%20slot%20at%20Loginn" target="_blank" class="btn btn-whatsapp"><i class="fa-brands fa-whatsapp"></i> Book a slot</a></aside></div></article>` });
}
